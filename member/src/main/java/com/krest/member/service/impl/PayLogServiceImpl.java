...
import com.krest.member.config.HttpClient;
import com.krest.member.entity.OrderVedio;
import com.krest.member.entity.PayLog;
import com.krest.member.mapper.PayLogMapper;
...
public class PayLogServiceImpl extends ServiceImpl<PayLogMapper, PayLog> implements PayLogService {
    ...
    public Map createNative(String orderNo) {
        ...
        try {
            //根据订单id获取订单信息
            OrderVedio order = orderVedioService.getById(orderNo);
            Map m = new HashMap();
            //1、设置支付参数
            m.put("appid", "wx74862e0dfcf69954");
            m.put("mch_id", "1558950191");
            m.put("nonce_str", WXPayUtil.generateNonceStr());
            m.put("body", order.getAlbumId());
            ...
            map.put("vedioSortId", order.getAlbumId());
    ...
    public void updateOrderStatus(Map<String, String> map) {
        ...
        if (status == null) {
            order.setStatus(1);
            orderVedioService.updateById(order);
        }
